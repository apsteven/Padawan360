// =======================================================================================
// ///////////////////////// Padawan360 Dome Code v1.7ser ///////////////////////////////////////
// =======================================================================================
//                                Steven Sloan
//                        Revised  Date: 03/Sep/2018
//                                Robert Corvus
...
//
// Logic Display and PSI Boards should be wired up in two chains (to prevent problems 
...
//  Arduino control PCB
  // 1= Using Teeces Rear Logic Display
  // 2= Using Arduino Pro Mini Breakout rev 1.6 by Mowee
...
#if (BOARDtype==1) && (CONTROLLERtype==1)
#define DVAL 12
#define CVAL 11
#define LVAL 10
#elif (BOARDtype==1) && (CONTROLLERtype==2) //Pinout slightly different for Arduino Pro Mini Breakout v1.6
#define DVAL 10
#define CVAL 11
#define LVAL 12
#elif (BOARDtype==2) 
...
#if (CONTROLLERtype==1)
#define RPSIval 3 //PSI is 4th device in the rear chain if Teeces Rear logic Board is used
#elif (CONTROLLERtype==2)
#define RPSIval 0 //PSI is 1st device in the rear chain if Arduino Pro Mini Breakout rev 1.6 is used
#endif
...
#elif (CONTROLLERtype==2)
  LedControl lcRear=LedControl(DVAL,CVAL,LVAL,1);  //rear chain,one device for the rear PSI
#endif
...
struct RECEIVE_DATA_STRUCTURE{
  ...
  int dsp; // 100=no change, 0=random, 1=alarm, 3=Hearts, 4=whistle, 5=leia, 6=short circut, 10=EQ, 11=alarm2, 21=speed1, 22=speed2, 23=speed3
};
...
void setup() 
{
 ...
 // exit shutdown of each chip and clear displays
  ...
  #endif
    lcRear.setIntensity(RPSIval, PSIbrightness); //Rear PSI
...
void setRPSI(byte mode)
{
  ...
  case OFF:
      for(byte row=0; row<HPROW; row++)
      {
        lcRear.setRow(RPSIval, row, 0x00);
      }
    break;
  case TEST:
      for(byte row=0; row<HPROW; row++)
      {
        lcRear.setRow(RPSIval, row, 0xFF);
      }
    break;
  case COLOR1:
      for(byte row=0; row<HPROW; row++)
      {
        lcRear.setRow(RPSIval, row, psiPatterns[row]);
      }
    break;
  case COLOR2:
      for(byte row=0; row<HPROW; row++)
      {
        lcRear.setRow(RPSIval, row, psiPatterns[row+5]);
      }
    break;
  default:
    break;
...
{   
  ...
  {
    psiMillisSwipe = currentMillis;
    lcRear.setRow(RPSIval, psiCurrentSwipeRow, psiPatterns[psiCurrentSwipeRow+psiColor]);   
    psiCurrentSwipeRow++; 
  }
  // if we are going to color 1 and haven't reached the first row, do next row
  else if (psiCurrentSwipeRow>=0 && psiColor == 0)
...
void showFailure(byte style)
{
  ...
  // RPSI
  for (int row=0; row<HPROW; row++)
    lcRear.setRow(RPSIval,row,randomRow(style));
}
